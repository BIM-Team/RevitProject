...
namespace Revit.Addin.RevitTooltip.Util
{
    public class SQLiteHelper
    {
        ...
        private static string dbPath = "SQLiteDB.db";
        ...
        //测试查询
        public Dictionary<string, string> selecttable()
        {
            Dictionary<string, string> data = new Dictionary<string, string>();
            string sql = "select b.ID,b.DATE,cast(b.VALUE - a.VALUE as DECIMAL(5,2)) from CXTable a, CXTable b where b.ID - a.ID =1 and b.ENTITY = a.ENTITY and b.ENTITY = 'CX5' ";

            //string sql = "select b.ID,b.DATE,b.VALUE from CXTable b where b.ENTITY = 'CX5' ";
            ...
            using (SQLiteCommand command = new SQLiteCommand(sql, conn))
            {
                ...
                try
                {
                    while (reader.Read())
                    {
                        if (reader.HasRows)
                        {
                            data.Add(reader.GetString(1), reader.GetFloat(2).ToString());
                        }
                    }
                    return data;
